function [ W2,B2,Centers,Spreads,scale ] = RBF2( SamIn,SamOut,Nc,w)
...
% Centers = [SamIn(1,A(1:ClusterNum));zeros(InDim-1,ClusterNum)];
ik=0;
while 1
    ...
    for i = 1:SamNum
        AllDistance = dist(Centers'./scale,SamIn(:,i)./(scale'));
...
AllDistances(I)=0.0001;
%Spreads = Overlap*min(AllDistances)'; 
Spreads = Overlap*mean(AllDistances)';
